    ...
    <div id="default-carousel" class="relative" data-carousel="static">
      ...
      <div class="overflow-hidden relative h-56 rounded-lg sm:h-64 xl:h-80 2xl:h-96">
        ...
        <div class="duration-700 ease-in-out" v-for="(image,ind) in form.images" :key="ind"
          ...
          <span
              class="absolute top-1/2 left-1/2 text-2xl font-semibold text-white -translate-x-1/2 -translate-y-1/2 sm:text-3xl dark:text-gray-800">alt image</span>
          <img :src="image"
      ...
      <div class="flex absolute bottom-5 left-1/2 z-30 space-x-3 -translate-x-1/2">
        <button v-for="(image,ind) in component.structure.props.images" :key="ind"
                class="w-3 h-3 rounded-full bg-gray-800/30"
                :class="{'bg-gray-800/60':ind===currentImageIndex}" @click="currentImageIndex=ind"></button>
      </div>
      <!-- Slider controls -->
      <button type="button"
              @click="toPrevImage"
